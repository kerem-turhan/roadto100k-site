...
- **Budget $0:** free tiers only — no paid fonts, APIs, analytics, or CDNs. Fonts are
  self-hosted via @fontsource. Exactly two external runtime surfaces are allowed: the
  Buttondown form action on submit and the cookieless GoatCounter script plus count endpoint
  (`src/lib/analytics.ts`, off unless `config.ANALYTICS_CODE` is set). Anything else is a new
